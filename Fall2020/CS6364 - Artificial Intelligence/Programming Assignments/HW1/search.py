...
def best_first_graph_search(problem, f, display=False):
    ...
    explored = set()
    print("Step 0 frontier list: ", frontier.heap)
    print("Step 0 explored list: ", explored)
    step_count = 0
    while frontier:
        node = frontier.pop()
        if step_count < 6:
            print(f"Step {step_count} current node: ", node, ", evaluation function(current):" , f(node))
        if problem.goal_test(node.state):
            if display:
        ...
        step_count += 1
        if step_count < 6:
            print(f"Step {step_count} frontier list: ", frontier.heap)
            print(f"Step {step_count} explored list: ", explored)
    return None
...
def recursive_best_first_search(problem, h=None):
    ...
    def RBFS(problem, node, flimit):
        ...
        while True:
            ...
            best = successors[0]
            if best.f > flimit:
                print(f"flimit:{flimit}, best:{best.f}, alternate:{None}, current node:{node.state}, next node:{None}, successors:{successors}")
                return None, best.f
            if len(successors) > 1:
                alternative = successors[1].f
            else:
                alternative = np.inf
            print(f"flimit:{flimit}, best:{best.f}, alternate:{alternative}, current node:{node.state}, next node:{best.state}, successors:{successors}")
