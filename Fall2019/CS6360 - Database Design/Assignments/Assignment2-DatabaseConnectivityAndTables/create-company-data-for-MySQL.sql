...
--GO in the next lines requried for :
--Each batch as a whole is parsed/validated against the state of the system at the time the batch runs (or would run). Since database_name doesn't exist before your original batch was started, you can't USE it as you did originally.
--By breaking it into two batches (by using the batch separator go), when USE database_name; comes along, it will exist because the first batch created it.
--I'm guessing that you're getting this from "Evaluate SQL" in SSMS, and not actually executing it; I think if you actually execute the whole original query, it'll work fine.
